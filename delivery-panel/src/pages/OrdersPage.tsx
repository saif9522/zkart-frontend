...
function directionsUrl(lat?: string | null, lng?: string | null, fallbackAddress?: string) {
  const dest = lat && lng ? `${lat},${lng}` : encodeURIComponent(fallbackAddress ?? '')
...
export function OrdersPage() {
  ...
  const renderActiveOrder = (order: MyDeliveryOrder) => {
    ...
    const goingToShop = order.status === 'ready'
    ...
    return (
      <div key={order.id} className="rounded-[var(--radius-card)] bg-rice-50 border border-ink-100/60 p-3.5 flex flex-col gap-2.5">
        ...
        {isCod && !goingToShop && (
          <div className="rounded-lg bg-mango-100 border border-mango-300 px-3 py-2 text-center">
            <p className="text-xs text-ink-400">Cash to collect from customer</p>
        ...
        <a
          href={
            goingToShop
              ? directionsUrl(order.vendor_latitude, order.vendor_longitude, order.vendor_address)
              : directionsUrl(order.customer_latitude, order.customer_longitude, order.delivery_address_text)
          }
          target="_blank"
          rel="noreferrer"
          className="flex items-center justify-center gap-2 rounded-lg bg-forest-600 text-rice-50 text-sm font-semibold py-2.5"
        >
          <Navigation className="h-4 w-4" /> {goingToShop ? 'Navigate to shop' : 'Navigate to customer'}
        </a>
        ...
        <div className="flex items-center justify-between text-xs text-ink-300">
          ...
          <span>{isCod ? 'Collect cash' : 'Prepaid'}</span>
        </div>
