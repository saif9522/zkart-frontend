...
import { Bike, Clock, IndianRupee, LogIn, LogOut, Package, PackageCheck, Star } from 'lucide-react'
      ...
      <div className="rounded-[var(--radius-card)] bg-rice-50 border border-ink-100/60 p-4 flex items-center gap-3">
        ...
        {data.rating?.average ? (
          <p className="text-sm text-ink-500">
            <span className="font-mono text-lg font-bold">{data.rating.average}</span> / 5 delivery rating
            <span className="text-ink-300"> · {data.rating.count} rating{data.rating.count === 1 ? '' : 's'}</span>
          </p>
        ) : (
          <p className="text-sm text-ink-300">No ratings yet — customers rate you after each delivery.</p>
        )}
      </div>
