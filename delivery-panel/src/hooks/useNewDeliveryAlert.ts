...
const SOUND_KEY = 'zkart-rider-sound'

/**
 * Polls the pool of claimable (READY) deliveries every 10s from anywhere in the app.
 ...
 */
export function useNewDeliveryAlert() {
  ...
  const { data } = useQuery({
    queryKey: ['available-orders', 'alert'],
