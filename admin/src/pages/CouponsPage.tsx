  ...
  const create = useMutation({
    mutationFn: () =>
      adminApi.createCoupon({
        ...
        // Date inputs give midnight; start of the "from" day and END of the "to" day,
        // so a coupon "valid to 31st" still works all day on the 31st.
    ...
    <div className="flex flex-col gap-4">
            ...
            {data?.results.map((c: Coupon) => (
              <tr key={c.id} className="border-b border-ink-100/60 last:border-0">
                ...
                <td className="px-4 py-3 font-mono text-ink-400">
                  ...
                  {c.per_user_limit ? <span className="block text-[11px] text-ink-300">max {c.per_user_limit}/customer</span> : null}
                </td>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="New coupon">
        ...
        >
          ...
          />
          <Field
            label="Uses per customer (1 = once per customer, blank = unlimited)"
            type="number"
