...
export function ProductDetailPage() {
  ...
  useSeo({
    title: product ? `${product.name}${product.unit ? ` (${product.unit})` : ''} — ₹${product.selling_price}` : undefined,
    description: product
      ? `Buy ${product.name} at ₹${product.selling_price}${Number(product.mrp) > Number(product.selling_price) ? ` (MRP ₹${product.mrp})` : ''} from ${product.vendor_name}. Fast delivery with zKart.shop. ${product.description_text ?? ''}`
      : undefined,
    image: primaryImage,
    type: 'product',
    jsonLd: product
      ? {
          ...
          description: (product.description_text || product.name).slice(0, 500),
          ...
          offers: {
            ...
            seller: { '@type': 'Organization', name: product.vendor_name },
