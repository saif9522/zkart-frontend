...
export function RegisterPage() {
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Brand new account — never has a shop yet, so go straight to onboarding.
      try {
        await vendorApi.profile()
        navigate('/')
      } catch {
        navigate('/onboard')
      }
    ...
    <div className="min-h-screen flex items-center justify-center px-4 bg-surface-900">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-rice-100/50">Vendor Panel</p>
        </div>

        {step === 'details' ? (
          <form onSubmit={handleSendOtp} className="rounded-[var(--radius-card)] bg-rice-50 p-6 flex flex-col gap-4">
            <div>
              <h2 className="font-semibold text-ink-500">Create your vendor account</h2>
